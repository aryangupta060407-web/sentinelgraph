...
export type EvidenceProviderResult = { type: string; response: string; source: "demo_evidence_provider" | "live_evidence_provider"; simulated: boolean; status: "received" | "unavailable"; reason: string };
export interface EvidenceProvider { request(type: string, context: Json): Promise<EvidenceProviderResult>; }

class DemoEvidenceProvider implements EvidenceProvider {
  async request(type: string): Promise<EvidenceProviderResult> { return { type, response: process.env.DEMO_EXTERNAL_RESPONSE && process.env.DEMO_EXTERNAL_RESPONSE !== "deterministic" ? process.env.DEMO_EXTERNAL_RESPONSE : "simulated_challenge_passed", source: "demo_evidence_provider", simulated: true, status: "received", reason: "Demo evidence provider response; no external verification was performed." }; }
}

class LiveEvidenceProvider implements EvidenceProvider {
  private endpoint = process.env.EVIDENCE_PROVIDER_URL || "";
  async request(type: string, context: Json): Promise<EvidenceProviderResult> {
    if (!this.endpoint) return { type, response: "unavailable", source: "live_evidence_provider", simulated: false, status: "unavailable", reason: "An approved external evidence provider is required; no endpoint is configured." };
    const response = await fetch(this.endpoint, { method: "POST", headers: { "Content-Type": "application/json", ...(process.env.EVIDENCE_PROVIDER_TOKEN ? { Authorization: `Bearer ${process.env.EVIDENCE_PROVIDER_TOKEN}` } : {}) }, body: JSON.stringify({ type, context }) });
    if (!response.ok) return { type, response: "unavailable", source: "live_evidence_provider", simulated: false, status: "unavailable", reason: `Approved evidence provider returned HTTP ${response.status}.` };
    const payload = await response.json() as Json;
    return { type, response: String(payload.response || payload.result || "received"), source: "live_evidence_provider", simulated: false, status: "received", reason: "Returned by the configured approved evidence provider." };
  }
}

function evidenceProvider(): EvidenceProvider { return process.env.EVIDENCE_PROVIDER_URL ? new LiveEvidenceProvider() : new DemoEvidenceProvider(); }
...
class TigerGraphRestTools implements GraphTools {
  ...
  async writeCase(caseId: string, state: Json) { const updated_at = new Date().toISOString(); const firstEvidence = state.evidence?.[0] || {}; const payload = { case_id: caseId, status: state.status || "open", verdict: state.verdict || "uncertain", probability: Number(state.probability || 0), pattern: state.pattern || "unclassified", exposure: Number(state.exposure || 0), summary: state.summary || "", approval_route: state.approval_route || "auto", selected_action: state.selected_action || "CREATE_CASE", action_status: state.action_status || (state.approval_route === "auto" ? "allowed" : "awaiting_approval"), stop_reason: state.stop_reason || "", evidence_summary: JSON.stringify(state.evidence || []), findings: JSON.stringify(state.findings || []), memory_summary: JSON.stringify(state.memory || []), updated_at, transaction_id: state.transaction_id || "", card_id: state.card_id || "", evidence_id: `${caseId}-evidence`, evidence_source: firstEvidence.source || "agent", evidence_query: firstEvidence.query || "investigation", evidence_claim: firstEvidence.claim || "Bounded investigation evidence", evidence_facts: JSON.stringify(firstEvidence.facts || state.evidence || []), evidence_strength: "moderate", event_id: `${caseId}-writeback`, event_step: "case_writeback", event_status: state.action_status || "recorded", event_detail: state.summary || "Case written", policy_id: `${caseId}-policy`, policy_text: `Action ${state.selected_action || "CREATE_CASE"}; approval route ${state.approval_route || "auto"}.` }; try { await this.query("write_case", payload); return { written: true, reference: caseId, source: "tigergraph" }; } catch (error) { return { written: false, reference: `tigergraph-write-failed:${caseId}`, source: `tigergraph_error:${error instanceof Error ? error.message : "unknown"}` }; } }
}
...
class TigerGraphMcpTools implements GraphTools {
  ...
  async writeCase(caseId: string, state: Json) { try { const data = await this.call("write_case", { case_id: caseId, ...state }); return { written: true, reference: JSON.stringify(data).slice(0, 220) || caseId, source: "tigergraph_mcp" }; } catch (error) { return { written: false, reference: `tigergraph-mcp-write-failed:${caseId}`, source: `tigergraph_mcp_error:${error instanceof Error ? error.message : "unknown"}` }; } }
}
...
type AgentPlan = { pattern: string; relevant_evidence_queries: string[]; next_tool: string; next_tool_reason: string; memory_influenced: boolean; memory_reason: string; needs_more_evidence: boolean; evidence_request_type: string; evidence_request_reason: string; recommended_action: string; action_reason: string; confidence_delta: number; explanation: string };

const agentPlanSchema = { type: "object", properties: {
  pattern: { type: "string" }, relevant_evidence_queries: { type: "array", items: { type: "string" } }, next_tool: { type: "string" }, next_tool_reason: { type: "string" }, memory_influenced: { type: "boolean" }, memory_reason: { type: "string" }, needs_more_evidence: { type: "boolean" }, evidence_request_type: { type: "string" }, evidence_request_reason: { type: "string" }, recommended_action: { type: "string" }, action_reason: { type: "string" }, confidence_delta: { type: "number" }, explanation: { type: "string" },
}, required: ["pattern", "relevant_evidence_queries", "next_tool", "next_tool_reason", "memory_influenced", "memory_reason", "needs_more_evidence", "evidence_request_type", "evidence_request_reason", "recommended_action", "action_reason", "confidence_delta", "explanation"], additionalProperties: false };
...
export async function investigateCase(caseId: string, trigger = "analyst_request"): Promise<InvestigationResult> {
  ...
  let providerResult: EvidenceProviderResult | null = null;
  ...
  if (requested) {
    ...
    providerResult = await evidenceProvider().request(requested.type, { case_id: caseId, customer_id: customer, transaction_id: txn, reason: requested.reason });
    trace.push({ time: now(), status: providerResult.simulated ? "simulated" : providerResult.status === "received" ? "complete" : "warning", step: "evidence_requested", label: `Requested ${requested.type}`, detail: providerResult.reason, source: providerResult.source });
    const response = providerResult.response;
    const delta = response.includes("failed") ? 0.12 : response.includes("passed") ? -0.1 : 0;
    finalProbability = clamp(initial + delta);
    finalConfidence = clamp(confidence + Math.max(0.1, reasoning.plan.confidence_delta || 0.16));
    trace.push({ time: now(), status: providerResult.status === "received" ? "complete" : "warning", step: "evidence_received", label: providerResult.simulated ? "Simulated evidence received" : "Live evidence received", detail: `Step-up response: ${response}`, source: providerResult.source });
  ...
  const stopReason = !gate.allowed ? "human approval required" : providerResult?.status === "unavailable" ? "additional evidence unavailable" : requested ? "conflicting evidence resolved" : reasoning.plan.next_tool === "none" ? "sufficient evidence" : "policy boundary reached";
  ...
  return { ...reference, case_id: caseId, agent: { mode: graphMode, llm_enabled: reasoning.enabled, trigger, transaction_id: txn, customer_id: customer, card_id: card, initial_confidence: confidence, final_confidence: finalConfidence, initial_probability: initial, final_probability: finalProbability, uncertainty, reasoning: reasoning.plan, memory_influenced: reasoning.plan.memory_influenced, memory_reason: reasoning.plan.memory_reason, stop_reason: stopReason, grounded_context: buildContext(), evidence, requested_evidence: requested ? [requested] : [], returned_evidence: providerResult ? [providerResult] : [], activity_trace: trace, policy: { action: finalAction, ...gate }, case_writeback: written, changed_recommendation: changed }, next_best_actions: { ...(reference.next_best_actions || {}), initial: [{ action: initialAction, route: policyGate(initial, 1 - confidence, initialAction).route, reason: reasoning.plan.action_reason }], final: [{ action: finalAction, route: gate.route, reason: gate.reason }], what_changed: changed }, case: { ...base, status: gate.allowed ? "monitoring" : "awaiting_approval", fraud_probability: finalProbability, pattern: reasoning.plan.pattern, stop_reason: stopReason, written_to_graph: written.written, graph_case_id: written.reference, evidence: [...(base.evidence || []), ...evidence.map((x) => ({ claim: x.claim, source: x.source, ref: x.query, entity_ids: x.entities, facts: x.facts }))], similar_prior_cases: memory?.entities || base.similar_prior_cases || [], summary: reasoning.plan.explanation } };
}
